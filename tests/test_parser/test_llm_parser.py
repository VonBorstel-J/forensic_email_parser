...
@pytest.fixture
def mocked_openai():
    with patch('src.parsers.llm_parser.openai.ChatCompletion.create') as mock_openai:
...
def test_llm_parser_openai_error(sample_email_content):
    with patch('src.parsers.llm_parser.openai.ChatCompletion.create', side_effect=openai.error.OpenAIError("API Error")):
        parser = LLMParser()
